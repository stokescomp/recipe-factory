...
class GoogleVoice
{
	...
	{
		if(isset($_SESSION['rnr']) AND $_SESSION['rnr'] > ''){
			$this->_rnr_se = $_SESSION['rnr'];
			//$this->_loggedIn = TRUE;
		}
		$this->_login = $login;
		$this->_pass = $pass;

		$this->_cookieFile = '/tmp/gvCookies';
	...
	{
if($this->_loggedIn) echo "you are logged in already so it will be fast<br />";
		//echo "credendials: ".$this->_rnr_se."<br />";
		if( $this->_loggedIn )
			return TRUE;
	...
	public function callNumber_widget($phone)
	{
		curl_setopt($this->_ch, CURLOPT_URL, 'https://clients4.google.com/voice/embed/webButtonConnect');
		curl_setopt($this->_ch, CURLOPT_POST, TRUE);
		curl_setopt($this->_ch, CURLOPT_POSTFIELDS, array(
		 'showCallerNumber' => '1',
		 'callerNumber' => "$phone",
		 'buttonId' => '9ea1f2e12e834c457a6a46434f9a73bef91d3166',
		 'name' => 'mike'
		 ));
	...
	{
	//echo "start loging in ".date('s',time())."<br /> ";
		$this->_logIn();
	//echo "logged in ".date('s',time())."<br /> ";
